...
* `Settings` reads the local `.env` unless tests explicitly pass `_env_file=None`; default-value tests and factory helpers must disable dotenv loading so an operator's live configuration cannot alter test expectations.
* OpenCodeGo requires an application-specific `User-Agent` and one stable `x-opencode-session` UUID per provider instance. Treat that UUID as sensitive session material: do not log, persist, fixture, or reuse it across processes.
